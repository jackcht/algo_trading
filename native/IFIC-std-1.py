...
# Deprecated : change to 'DataFrame.rolling(window=10,center=False).mean()'
mean_sp = pd.rolling_mean(spread, rolling_minutes).dropna()
#mean_sp.to_csv('mean_this.csv')

# Deprecated : change to 'DataFrame.rolling(window=10,center=False).std()'
std_sp = pd.rolling_std(spread, rolling_minutes).dropna()
#std_sp.to_csv('std_this.csv')

upper_sp = (mean_sp + std_sp * entry_bound_level1[1]).values
upper_exit_sp = (mean_sp + std_sp * exit_bound[1]).values
lower_sp = (mean_sp + std_sp * entry_bound_level1[0]).values
lower_exit_sp = (mean_sp + std_sp * exit_bound[0]).values

assert(len(mean_sp) == len(std_sp))
spread = spread[rolling_minutes-1:]
print len(spread)                   # 5633
print len(mean_sp)                  # 5633
